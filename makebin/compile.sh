...
CGO_ENABLED=0 GOOS=${GOOS} GOARCH=amd64 go build -a -compiler gc -installsuffix cgo -ldflags '-d -s -w' -o "/tmp/goscm_build/${GOOS}/scm-tarball" cmd/scm-tarball/main.go
mkdir -p "tmp/${GOOS}"
cp "/tmp/goscm_build/${GOOS}/scm-clone" "tmp/${GOOS}/scm-clone"
cp "/tmp/goscm_build/${GOOS}/scm-tarball" "tmp/${GOOS}/scm-tarball"
echo "tmp/${GOOS}"
ls -lh "tmp/${GOOS}"
